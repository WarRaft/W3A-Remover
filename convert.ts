...
function main() {
	...
	if (ext === ".w3a") {
		...
		const conversionResult = Translator.ObjectsTranslator.warToJson(
			objectType,
			fileBuffer
		);
		...
		const conversionResult = Translator.ObjectsTranslator.jsonToWar(
			objectType,
			jsonData
		);
		...
		if (outputFile) {
			...
			console.log(
				`Converted ${inputFile} (JSON) to ${defaultOutput} (w3a)`
			);
		}
	} else {
		console.error(
			"Unsupported file extension. Provide a .w3a file for conversion to JSON or a .json file for conversion to w3a."
		);
		process.exit(1);
	}
}
